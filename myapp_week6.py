...
# def create_a_records(bandname):
#     # if not request.json or not 'name' in request.json:
#     #     return jsonify({'error':'the new record needs to have a band name'}),400
#     for item in all_records:
#         if item['name']==bandname:
#             new_record = {'title':request.json['title'],'songs':request.json.get('songs','')}
#             item['albums'].append(new_record)
#     return jsonify({'message':'created: /records/{}'.format(new_record['title'])}),201
